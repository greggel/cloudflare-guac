...
### edit your local .env file to include CLOUDFLARETEAM={enter value here} before running
ENV CLOUDFLARETEAM=${CLOUDFLARETEAM}
...
#Install CloudflareD - Fetch go code
WORKDIR /root
RUN wget https://github.com/cloudflare/cloudflared/releases/download/2023.8.2/cloudflared-linux-amd64 -O cloudflared
...
COPY --from=builder /root/cloudflared /usr/local/bin/cloudflared
